...
def simple_mpls_packet(pktlen=100, 
                       eth_dst='00:01:02:03:04:05',
                       eth_src='00:06:07:08:09:0a',
                       mpls_labels=[],
                       dl_vlan_enable=False,
    ...
    @param eth_dst Destinatino MAC
    @param eth_src Source MAC
    @param mpls_labes Lables to add to the payload in order of ETH|N|N-1|N-2|...|IP
    @param dl_vlan_enable True if the packet is with vlan, False otherwise
    @param vlan_vid VLAN ID
    @param vlan_pcp VLAN priority
    @param ip_src IP source
    ...
    if (dl_vlan_enable):
        ...
        pkt /= scapy.IP(src=ip_src, dst=ip_dst, tos=ip_tos, ttl=ip_ttl, ihl=ip_ihl)
        pkt /= scapy.TCP(sport=tcp_sport, dport=tcp_dport, flags=tcp_flags)
    else:
        if not ip_options:
            pkt = scapy.Ether(dst=eth_dst, src=eth_src)
            ...
            for label in mpls_labels:
                pkt /= label
            pkt /= scapy.IP(src=ip_src, dst=ip_dst, tos=ip_tos, ttl=ip_ttl, ihl=ip_ihl, options=ip_options)
            pkt /= scapy.TCP(sport=tcp_sport, dport=tcp_dport, flags=tcp_flags)
...
def verify_packet_in(test, data, in_port, reason, controller=None):
    ...
    while True:
        ...
        msg, _ = controller.poll(ofp.OFPT_PACKET_IN, 20)
        if not msg:
            # Timeout
            break
...
def verify_packet(test, pkt, ofport):
    ...
    #(rcv_port, rcv_pkt, pkt_time) = test.dataplane.poll()
    (rcv_port, rcv_pkt, pkt_time) = test.dataplane.poll(exp_pkt=str(pkt), timeout=20)
    #(rcv_port, rcv_pkt, pkt_time) = test.dataplane.poll(exp_pkt=str(pkt), timeout=5)

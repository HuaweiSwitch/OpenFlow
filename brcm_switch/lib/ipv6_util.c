...
#define LOG_MODULE VLM_ipv6_util
#include "vlog.h"
...
lookup_ipv6(const char *host_name, struct in6_addr *addr)
{
    if (inet_pton(AF_INET6, host_name, addr) != 1) {
        VLOG_ERR(LOG_MODULE, "%s is not a valid IPv6 address \n", host_name);
        return ENOENT;
    }
    return 0;
}
...
ipv6_create_mask(int mask)
{
    ...
    unsigned char *netmaskp = &netmask.s6_addr[0];
...
str_to_ipv6(const char *str_, struct in6_addr *addrp, struct in6_addr *maskp)
{
    ...
    if (retval) {
        VLOG_ERR(LOG_MODULE, "%s: could not convert to IPv6 address\n", str);
        /* 释放内存 */
        free(str);
        return -1;
    }
    ...
    if (netmask) {
        ...
        if (prefix <= 0 || prefix > 128) {
            VLOG_ERR(LOG_MODULE, "%s: network prefix bits not between 1 and 128\n",
                     str);
            /* 释放内存 */
            free(str);
            return -1;

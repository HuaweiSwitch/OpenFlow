...
class PushVlan(BaseModifyPacketTest):
    ...
    def runTest(self):
        actions = [ofp.action.push_vlan(ethertype=0x8100),
                   ofp.action.set_field(ofp.oxm.vlan_vid(100))]
        pkt = simple_tcp_packet()
...
class PopVlan(BaseModifyPacketTest):
    ...
    def runTest(self):
        ...
        pkt = simple_tcp_packet(dl_vlan_enable=True, vlan_vid=3, pktlen=104)
...
class SetVlanVid(BaseModifyPacketTest):
    ...
    def runTest(self):
        ...
        pkt = simple_tcp_packet(dl_vlan_enable=True, vlan_vid=10)
...
class SetVlanPcp(BaseModifyPacketTest):
    ...
    def runTest(self):
        ...
        pkt = simple_tcp_packet(dl_vlan_enable=True, vlan_vid=10, vlan_pcp=1)
        exp_pkt = simple_tcp_packet(dl_vlan_enable=True, vlan_vid=10, vlan_pcp=2)
        self.verify_modify(actions, pkt, exp_pkt)

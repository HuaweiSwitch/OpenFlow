...
WILDCARD_VALUES = [ 1 << ofp.OFPXMT_OFB_IN_PORT,
                    ...
                    1 << ofp.OFPXMT_OFB_MPLS_LABEL,
                    1 << ofp.OFPXMT_OFB_MPLS_TC]
"""
...
#    if (sup_acts & 1<<ofp.OFPAT_SET_FIELD):
...
def mpls_set_label_act_tests(parent, test_condition=0):
    ...
    if test_condition == 0:
        ...
        act.field = match.mpls_label(parent.label + 2)
        if parent.num_tags == 0:
            match_exp = False
            exp_msg = ofp.OFPT_ERROR
            exp_msg_type = ofp.OFPET_BAD_ACTION
        ...
        act.field = match.mpls_label(1048576)
        if parent.num_tags == 0:
            match_exp = False
            exp_msg = ofp.OFPT_ERROR
            exp_msg_type = ofp.OFPET_BAD_ACTION

...
enum ofp_extension_commands { /* Queue configuration commands */
    ...
    OFP_EXT_MOD_QOS_GROUP,
    OFP_EXT_MAP_QUEUE,

    OFP_EXT_MOD_PORT_CONF,
    OFP_EXT_SET_NETWORK
};

struct ofp_extension_header {
    struct ofp_header header;
    unsigned int vendor;            /* OPENFLOW_VENDOR_ID. */
    unsigned int subtype;           /* One of ofp_extension_commands */
};
...
struct openflow_queue_command_header {
    ...
    unsigned int port;              /* Port for operations */
    unsigned char pad[4];             /* Align to 64-bits */
    unsigned char body[0];            /* Body of ofp_queue objects for op. */
};
...
struct openflow_ext_show_perf{
    struct ofp_extension_header header;
    unsigned int command;
};
OFP_ASSERT(sizeof(struct openflow_ext_show_perf) == 20);


enum openflow_hw_ext_config
{
    HEC_SOFT_ENABLE   = 1 << 0,               /* disable  brcm  dataplane */
    HEC_ALTA_PORT_RESET = 1 << 1              /* Clear brcm port's vlan config */
};

struct openflow_ext_hw_config
{
    struct ofp_extension_header header;
    unsigned int config;                    /* HEC_* flags. */
};
OFP_ASSERT(sizeof(struct openflow_ext_hw_config) == 20);

struct openflow_ext_mod_port_conf{
    struct ofp_extension_header   header;
...
struct openflow_ext_network_conf {
    struct ofp_extension_header   header;
...
struct openflow_ext_mod_qos_group
{
    struct ofp_extension_header header;
...
struct ofp_queue_prop_buf_alloc {
    ...
    unsigned short int alloc_val;       /* 0 disabled; 1 min; 0xffff max */
...
struct ofp_queue_prop_sched_weight {
    struct ofp_queue_prop_header prop_header; /* prop: OFPQT_MIN, len: 16 */
    unsigned short int weight;   /* discipline specific; 0 disabled; 1 min; 0xffff max */
    unsigned char pad[6];    /* 64-bit alignment */
};

...
unsigned int hash_words(const unsigned int *, size_t n_word, unsigned int basis);
unsigned int hash_2words(unsigned int, unsigned int);
unsigned int hash_3words(unsigned int, unsigned int, unsigned int);
unsigned int hash_bytes(const void *, size_t n_bytes, unsigned int basis);

static inline unsigned int hash_string(const char *s, unsigned int basis)
{
 ...
 * quality. */
static inline unsigned int hash_int(unsigned int x, unsigned int basis)
{
    x -= x << 6;
    x ^= x >> 17;
    x -= x << 9;
...
static inline unsigned int hash_double(double x, unsigned int basis)
{
...
static inline unsigned int hash_pointer(const void *p, unsigned int basis)
{
    ...
    return hash_int((unsigned int) (uintptr_t) p, basis);
}
